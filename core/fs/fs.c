...
<----------+---------------------+ +------> sector 0
           ...
           |                     |
           +---------------------+
           +---------------------+ +---------> sector: block_bitmap_lba
           |                     |
           |                     |
           +--                 --+ +---------> sector: block_bitmap_lba+ block_bitmap_sects
           |                     |
           +---------------------+
           +---------------------+ +---------> sector: inode_bitmap_lba
           |                     |
           |                     |
           +--                 --+ +---------> sector: inode_bitmap_lba+inode_bitmap_sects
           |                     |
           +---------------------+
           +---------------------+ +---------> sector: inode_table_lba
           |     inode0          |
           +----               --+
           +----               --+ +---------> sector: inode_table_lba+inode_table_sects
           |     inode1          |
           +----               +-+
           |    more_inodes....  |
           +---------------------+
...
static void partition_format(struct partition *part)
{
    ...
    // 第0块是引导块,第1块是超级块，块 位图从第 2 个扇区开始
    ...
    ******************************/
    ...
    /*
     找出数据量最大的元信息,用其尺寸做存储缓冲区
      （元信息需要几十上百个扇区大小 对应的内存。 栈中放不下，需要在堆中申请
    */
